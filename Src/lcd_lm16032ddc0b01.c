...
#define GRAPHIC_CURSOR_HEIGHT   3
static const uint8_t graphicsCursor[GRAPHIC_CURSOR_HEIGHT] = {
    0x10, 0x38, 0x7c
};

#define LCD_DISPLAY_BUFFER_LEN (CHAR_PER_LINE*2 + 1)
static uint8_t lcd_display_buf[LCD_DISPLAY_BUFFER_LEN];
...
{
	if(pos % 2)return;
	lcdWriteCommand(CMD_BASIC_SET_DDRAM_ADDR  | (line << 4) |  (pos >> 1));
}
...
{
    lcdSetPin(LCD_BKLED, (status == BKLED_OFF) ? 0 : 1);
    delayNOP();
}

static void lcdInitCG(void)
{
#if 0
	const uint8_t* font;
	/*精简指令集下不用考虑SR*/
	lcdWriteCommand(CMD_BASIC_FUNCTION_SET | FUNSET_DL_8BIT);
	
    for(uint8_t i = 0; i < 16; ++i){
        lcdSetCGRAMAddr(idx, i);
        lcdWriteData((uint8_t)(font[i]>>8));
        lcdWriteData((uint8_t)font[i]);
    }
#endif	
}
...
uint8_t lcdGetChar(uint8_t line, uint8_t pos)
{
    ...
    if(pos > CHAR_PER_LINE){
	    printf("pos error\n");
	    return 0;
	}

    return lcd_display_buf[pos];
}

void lcdSetChar(uint8_t line, uint8_t pos, const uint8_t data)
{
    uint8_t i = 0;
    lcd_display_buf[pos] = data;
    ...
    for(i = 0; i < CHAR_PER_LINE; ++i){
        lcdWriteData(lcd_display_buf[i]);
    }
    return;
}
...
void lcdDisplay(uint8_t line, const uint8_t *buffer)
{
    ...
    memset(lcd_display_buf, 0, LCD_DISPLAY_BUFFER_LEN);

    while (buffer[i]) {
        lcd_display_buf[i] = buffer[i];
...
{
    uint32_t i = 0, j = 0;
    lcdEisEnter(true, true);
    for(i = 0; i < CHAR_PER_LINE; ++i){
		for(j = 0; j < 16; ++j){
			lcdSetGDRAMPos(i, 15 - j);
			lcdWriteData(0);		//先写高字节
			lcdWriteData(0);		//再写低字节
			lcdSetGDRAMPos(i, 31 - j);
			lcdWriteData(0);		//先写高字节
			lcdWriteData(0);		//再写低字节
		}
	}
...
{
    uint32_t i = 0;
    const uint8_t* cursor = graphicsCursor;
    if(line > LINE_TWO || pos > CHAR_PER_LINE){
        printf("param error\n");
        return;
    }

    lcdInitCursor();    /*clear last cursor*/

    lcdEisEnter(true, true);
    for(i = 0; i < GRAPHIC_CURSOR_HEIGHT; ++i){
        lcdSetGDRAMPos(pos / 2, (line + 1)* 16 - 1 -i);

        if(pos % 2){
            lcdWriteData(0);
            lcdWriteData(cursor[GRAPHIC_CURSOR_HEIGHT - 1 - i]);
        }else{
            lcdWriteData(cursor[GRAPHIC_CURSOR_HEIGHT - 1 - i]);
            lcdWriteData(0);    
        }
